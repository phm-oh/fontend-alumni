...
const AdminDashboard = ({ user, onLogout, onNavigate }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const shippingRes = await fetch(`${import.meta.env.VITE_API_URL}/api/shipping/statistics`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const calculateShippingStats = async (token) => {
    try {
      ...
      const approvedRes = await fetch(`${import.meta.env.VITE_API_URL}/api/alumni?status=อนุมัติ&limit=100`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (approvedRes.ok) {
        ...
        const shippingStats = {
          needShipping: approvedAlumni.filter(a => a.deliveryOption === 'จัดส่งทางไปรษณีย์').length,
          pending: approvedAlumni.filter(a => 
            a.deliveryOption === 'จัดส่งทางไปรษณีย์' && 
            (a.shippingStatus === 'รอการจัดส่ง' || !a.shippingStatus)
          ).length,
          shipping: approvedAlumni.filter(a => a.shippingStatus === 'กำลังจัดส่ง').length,
          delivered: approvedAlumni.filter(a => a.shippingStatus === 'จัดส่งแล้ว').length
        };
  ...
  const StatCard = ({ title, value, icon, color = "blue", change, onClick, subtitle }) => (
                      ...
                      <div className="shipping-info">
                        ...
                        {alumni.deliveryOption === 'จัดส่งทางไปรษณีย์' && (
                          <div className="shipping-status">
                            <span className={`status-badge ${alumni.shippingStatus === 'จัดส่งแล้ว' ? 'green' : 
                                                            alumni.shippingStatus === 'กำลังจัดส่ง' ? 'blue' : 'yellow'}`}>
